...
def simulate_chargepoint_with_DTS(ticks_per_year, ticks_per_hour, chargepoint_states, total_energy_per_tick, power_demand_per_tick, charging_power, kwh_per_100km, charging_events, arrival_multiplier):
    ...
    for tick in range(ticks_per_year):
        ...
        if chargepoint_states[tick] is False:
            ...
            adjusted_arrival_probability = arrival_probabilities[hour_of_day] * arrival_multiplier / 100
            if random.random() < adjusted_arrival_probability:
                ...
                if charging_demand > 0:
                    ...
                    print("CHARGING_EVENT|%s" % json.dumps({
                        'time': date.isoformat(),
...
def main():
    ...
    parser.add_argument('--arrival_multiplier', type=float, default=100, help='Arrival multiplier as a percentage (20-200)')

    args = parser.parse_args()

    run(args.num_chargepoints, args.charging_power, args.car_consumption, args.arrival_multiplier)
